...
from Dataset.globals import REALWORLD_DATA_DIR
...
class RealWorldDataset_4dof_pcd(Dataset):
    ...
    # def get_relevant_data(self, data: dict):
    #     crop_data = {}
    ...
    #     crop_data["depth_0"] = crop_data["depth_0"] * (crop_data["depth_0"] < 1.2)
    #     crop_data["depth_1"] = crop_data["depth_1"] * (crop_data["depth_1"] < 1.2)
    ...
    #     # crop_data["depth_0"] = depth0
    #     # crop_data["depth_1"] = depth1
    #     # crop_data["seg_0"] = segmap0
    #     # crop_data["seg_1"] = segmap1
    #     # crop_data["rgb_0"] = rgb0
    #     # crop_data["rgb_1"] = rgb1
    #     # crop_data["intrinsics_0"] = data["intrinsic"].copy()
    #     # crop_data["intrinsics_1"] = data["intrinsic"].copy()

    #     # data["T_WC_opencv"][2,3] -= 0.5
    #     crop_data["T_WC"] = data["T_WC_opencv"]
    ...
    # def get_filtered_pointcloud_ids(self, pcd):
    #     n_smallest = 50
    #     std_scale = 2

    #     semi_sorted_args = np.argpartition(-pcd[:,2], n_smallest) #get arg of 50 largest values
    #     largest_values = pcd[semi_sorted_args[:n_smallest], 2]
    #     print(f"\n\n\nLargest: {largest_values}")
    #     mean, std = np.mean(largest_values), np.std(largest_values)
    #     keep_args = pcd[:,2] < (mean + std_scale * std)
    #     return keep_args
    ...
    def get_pointclouds(self, crop_data: dict):
        ...
        # print(self.get_filtered_depth_ids(crop_data["depth_0"], crop_data["seg_0"]).astype(int) + valid_keypoints.astype(int))
        # crop_data['pc0_keep_id'] = self.get_filtered_pointcloud_ids(projected_keypoints)
        crop_data['pc0_keep_id'] = (self.get_filtered_depth_ids(crop_data["depth_0"], crop_data["seg_0"]).astype(int) + valid_keypoints.astype(int)) == 2
        ...
        crop_data['pc1_keep_id'] = (self.get_filtered_depth_ids(crop_data["depth_1"], crop_data["seg_1"]).astype(int) + valid_keypoints.astype(int)) == 2
        projected_keypoints = np.concatenate((projected_keypoints, np.ones((projected_keypoints.shape[0], 1))), axis=1).transpose(1,0)
    ...
    def get_transformation(self, data: dict):
        ...
        T_delta_cam = data["T_delta_cam"]
        T_delta_base = data["T_delta_base"]      
    ...
    def __getitem__(self, idx):
        ...
        self.idx = idx #// 2
        flip_object_pairs = 1# (idx % 2)*-2 + 1  #1 if even, -1 if odd
...
if __name__ == "__main__":
    ...
    def get_transformation(data):
        ...
        pc0_centre = T[:3,:3] @ np.mean(data["pc0"][:3,:], axis=1)
        pc1_centre = np.mean(data["pc1"][:3,:], axis=1)
        # T[:3,3] = data["t_label"] - t_rotated[:,0]
        T[:3,3] = pc1_centre - pc0_centre
        return T
    ...
    dataset = RealWorldDataset_4dof_pcd('reg', n_bins=90, debug=True)
