...
class DataAugmentator():
    ...
    def __init__(self, p_rgb=0.35, p_depth=0.5) -> None:
        ...
        # self.rgb_p[6] = 1
        self.depth_p = np.array([0.5, 0.5]) * p_depth
    ...
    def manipulate_depth(self, images: np.ndarray):
        random_num = np.random.rand()
        cumulative_prob = 0
        for i in range(len(self.depth_p)):
            cumulative_prob += self.depth_p[i]
            if random_num < cumulative_prob:
                if i == 0:
                    noise = np.random.normal(0, 0.005, images.shape)
                elif i == 1:
    ...
    def augment_data(self, rgb, depth):
        stacked_rgb = np.concatenate((rgb[0][None], rgb[1][None]), axis=0)
        stacked_depth = np.concatenate((depth[0][None], depth[1][None]), axis=0)
        ...
        aug_rgb = self.manipulate_rgb(stacked_rgb)
        aug_depth = self.manipulate_depth(stacked_depth)
        return [aug_rgb[0], aug_rgb[1]], [aug_depth[0], aug_depth[1]]
