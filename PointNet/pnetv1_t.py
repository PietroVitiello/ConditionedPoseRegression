...
class PointNetv1_tReg(nn.Module):

    def __init__(self, only_translation: bool = False) -> None:
        super(PointNetv1_tReg, self).__init__()
        ...
        self.fusion = nn.Sequential(nn.Linear(in_features=518, out_features=256, bias=True),
                                    # nn.BatchNorm1d(256),
                                    nn.InstanceNorm1d(256),
                                    nn.LeakyReLU(inplace=True))
    ...
    def forward(self, batch: dict, rot_pred: float = None) -> torch.Tensor:
        ...
        out = torch.concat((encoded_live, encoded_bottleneck, torch.mean(pcd0[:,:3,:], dim=2), torch.mean(batch["pc1"][:,:3,:], dim=2)), dim=1)
        out = self.fusion(out)
        out = self.mlp2(out)
        out = self.mlp3(out)
