...
class PointNetv4_tReg(nn.Module):

    def __init__(self, only_translation: bool = False) -> None:
        super(PointNetv4_tReg, self).__init__()
        ...
        self.obj_residual_perceptron = nn.Linear(in_features=512, out_features=3, bias=True)
    ...
    def forward(self, batch: dict, rot_pred: float = None) -> torch.Tensor:
        ...
        pcd1 = batch["pc1"].clone()
        if self.only_translation:
            rot_pred = torch.rand(pcd0.shape[0], device=pcd0.device) * 0.7 - 0.35 + batch["rot_label"]
            batch['rot_pred'] = rot_pred
        if rot_pred is not None:
            pcd0 = self.rotate_pointcloud(pcd0, rot_pred)

        rotated_pcd0_centre = torch.mean(pcd0[:,:3,:], dim=2)
        pcd0[:,:3,:] = pcd0[:,:3,:] - rotated_pcd0_centre[...,None].expand(-1,-1,pcd0.shape[2])
        pcd0[:,-3:,:] = pcd0[:,:3,:]
        pcd1[:,:3,:] = pcd1[:,:3,:] - torch.mean(pcd1[:,:3,:], dim=2)[...,None].expand(-1,-1,pcd0.shape[2])
        pcd1[:,-3:,:] = pcd1[:,:3,:]

        encoded_live = self.encoder(pcd0)
        encoded_bottleneck = self.encoder(pcd1)

        pc01 = torch.concat((encoded_live, encoded_bottleneck), dim=1)
        pc10 = torch.concat((encoded_bottleneck, encoded_live), dim=1)

        obj0_residual_pred = self.obj_residual_perceptron(pc01)
        obj1_residual_pred = self.obj_residual_perceptron(pc10)
        # residual_translation_pred = self.res_translation_perceptron(pc10)

        batch["rotated_pc0_centre"] = rotated_pcd0_centre
